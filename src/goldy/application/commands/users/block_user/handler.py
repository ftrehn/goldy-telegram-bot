...
class BlockUserHandler(CommandHandler[BlockUserCommand, UserView]):
    """Blocks someone the caller outranks.

    ``CanManageSubordinate`` alone covers more than it looks: it refuses equals
    as well as superiors, so an administrator cannot block a peer, and — since
    nobody outranks themselves — cannot lock themselves out either.
    """
    ...
    @override
    async def handle(self, command: BlockUserCommand) -> UserView:
        ...
        target.block(BlockReason(value=command.reason))
