...
the total does about it — all of it the mapper's, injected into the gateway
rather than written inside it.
"""
...
from tests.unit.factories.cart_factories import make_cart_line_row

MAPPER = SqlAlchemyCartRowViewMapper()
...
def test_an_empty_cart_comes_to_nothing_rather_than_to_no_total() -> None:
    ...
    total = MAPPER.to_cart_view(()).total
...
def test_the_total_is_the_sum_of_the_lines_that_have_a_price() -> None:
    rows = (
        make_cart_line_row(quantity=3, price="10.00"),
        make_cart_line_row(index=2, quantity=2, price="19.99"),
    )

    total = MAPPER.to_cart_view(rows).total
...
def test_an_unpriced_line_adds_nothing_and_does_not_break_the_total() -> None:
    ...
    rows = (
        make_cart_line_row(quantity=1, price="10.00"),
        make_cart_line_row(index=2, quantity=5, price=None),
    )
...
def test_a_line_left_by_a_withdrawn_product_still_counts() -> None:
    ...
    rows = (make_cart_line_row(quantity=2, price="10.00", is_active=False),)
...
def test_a_cart_of_nothing_but_unpriced_lines_still_has_a_total() -> None:
    ...
    rows = (make_cart_line_row(price=None),)
...
def test_a_drawn_line_multiplies_the_price_the_projection_holds_today() -> None:
    ...
    line = MAPPER._to_line_view(make_cart_line_row(quantity=3, price="19.99"))
...
def test_the_value_objects_the_cart_owns_are_unwrapped_and_the_rest_is_not() -> None:
    ...
    """
    line = MAPPER._to_line_view(make_cart_line_row(index=7))
...
def test_a_line_with_no_price_under_this_price_type_loses_only_its_price() -> None:
    ...
    line = MAPPER._to_line_view(make_cart_line_row(quantity=4, price=None))
...
def test_a_product_withdrawn_from_the_catalog_is_marked_and_not_hidden() -> None:
    ...
    line = MAPPER._to_line_view(make_cart_line_row(is_active=False))
...
def test_a_product_the_projection_no_longer_holds_at_all_still_draws() -> None:
    ...
    line = MAPPER._to_line_view(make_cart_line_row(product=False))
...
def test_stock_is_a_badge_and_never_a_reason_to_hide_a_line() -> None:
    ...
    stocked = MAPPER._to_line_view(make_cart_line_row(stock="12.500"))
    empty = MAPPER._to_line_view(make_cart_line_row(stock="0"))
