...
checkout-address-prompt = Where should it go? Send the address as one line: city, street, building, flat. We deliver within Russia only — write the address in Cyrillic, with a building number.
...
checkout-comment-prompt = A note for the order — a convenient delivery time, for instance, up to { $max_length } characters. Or skip this step.
