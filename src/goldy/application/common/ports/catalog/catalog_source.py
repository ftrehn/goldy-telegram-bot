...
class CatalogSource(Protocol):
    ...
    **A pulling port, and that is a detail of the CLI seeder rather than the
    seam of the integration.** The HTTP receiver 1C posts to
    (``infrastructure/catalog_receiver``, ADR-0004) pushes: it is handed a
    request body and cannot implement an interface the bot polls, and it
    should not try.

    The seam is ``ImportCatalogCommand(CatalogSnapshot)``. The receiver builds
    the snapshot out of the request body and sends the command; this port
